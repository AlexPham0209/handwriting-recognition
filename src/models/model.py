...
class CaptchaDetectionModel(nn.Module):
    def __init__(self, num_chars, device):
        ...
        self.conv = nn.Sequential(
            ConvBlock(3, 64, device).to(device),
            ConvBlock(64, 128, device).to(device),
            ConvBlock(128, 256, device, (2, 2)).to(device),
            ConvBlock(256, 256, device, (2, 2)).to(device),
        ).to(device)

        self.fc1 = nn.Linear(in_features=22, out_features=32).to(device)
